...
BT_Model = function(matrix, Model_type, R = NULL){
  # Bradley Terry Model 
  # Returns the 'thetas' with the least parameter set to 0
  # Specify the type of Model in this manner
  ...
  if(Model_type == 'VAN'){
    ...
    return(list(Table = Table, type = Model_type, Worst = least, Best = best, Teams = teams))
  }
  ...
  if(Model_type == 'CHI'){
    ...
    Theta = (data.frame(Theta = theta, row.names = teams))
    Alpha = data.frame(Alpha = alpha, row.names = 1:max(R))
    
    return(list(Theta = Theta, Alpha = Alpha, type = Model_type, Advantage = bigger_advantage, Teams = teams))
  }
  ...
  if(Model_type == 'TSH'){
    ...
    return(list(Theta = Theta, Alpha = Alpha, type = Model_type, Highest_Home_Advantage = bigger_advantage, Teams = teams))
  }
...
BT_plots = function(model){
  ## Takes in a Bradley-Terry model from the BT_Model function and produces a plot of the theta values by team.
  
  ## For hierarchical models, it prints each plot every ten seconds, starting with R_1, up to R_n
...
BT_predict = function(model, df, rel = NULL){
  ...
  for (i in 1:nrow(df)){
    if(df[i,1] %in% model$Teams & df[i,2] %in% model$Teams){
      if(df[i,3] != df[i,4]){
        ...
        if(model$type == 'CHI'){
          Theta <- model$Theta
          df$Team_B_win[i] = exp(Theta[df[i,2],] + model$Alpha[R[df[i,2],df[i,1]],])/(exp(Theta[df[i,2],] + model$Alpha[R[df[i,2],df[i,1]],]) + exp(Theta[df[i,1],]))
          df$Team_A_win[i] = 1 - df$Team_B_win[i]
        }
        if(model$type == 'TSH'){
          Theta <- model$Theta
          Alpha <- model$Alpha
          df$Team_B_win[i] = exp(Theta[df[i,2],] + Alpha[df[i,2],])/(exp(Theta[df[i,2],] + Alpha[df[i,2],]) + exp(Theta[df[i,1],]))
          df$Team_A_win[i] = 1 - df$Team_B_win[i]
        }
        if(model$type == 'HIE'){
          Theta <- model$Theta
          Alpha <- model$Alpha
          df$Team_B_win[i] = exp(Theta[df[i,2],] + model$Alpha[df[i,2], R[df[i,2],df[i,1]]])/(exp(Theta[df[i,2],] + model$Alpha[df[i,2], R[df[i,2],df[i,1]]]) + exp(Theta[df[i,1],]))
          df$Team_A_win[i] = 1 - df$Team_B_win[i]
        }
      } 
      if(df[i,3] == df[i,4]){
        ...
        if(model$type == 'CHI'){
          Theta <- model$Theta
          df$Team_B_win[i] = exp(Theta[df[i,2],])/(exp(Theta[df[i,2],]) + exp(Theta[df[i,1],]))
          df$Team_A_win[i] = 1 - df$Team_B_win[i]
        }
        if(model$type == 'TSH'){
          Theta <- model$Theta
        ...
        if(model$type == 'HIE'){
          Theta <- model$Theta
          Alpha <- model$Alpha
          df$TeamB_win[i] = exp(Theta[df[i,2],])/(exp(Theta[df[i,2],]) + exp(Theta[df[i,1],]))
          df$Team_A_win[i] = 1 - df$Team_B_win[i]
        }
      }
    }
